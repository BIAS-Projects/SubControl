...
public interface ISerialWorker : IAsyncDisposable
{
    ...
    Task<OperationResult> WriteAsync(byte[] data, CancellationToken token = default);

    /// <summary>Convenience overload for text workers.</summary>
    Task<OperationResult> WriteTextAsync(string text, CancellationToken token = default);
